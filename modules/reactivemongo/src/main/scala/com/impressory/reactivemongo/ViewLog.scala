...
object ViewLog {
  ...
  def log(record:Record) = {
    ...
    val incSFs:Seq[(String, BSONInteger)] = for (e <- record.entry.toSeq; v = e.settings.showFirst) yield (s"hourly.${hour}.settings.showFirst.${v.toString}" -> BSONInteger(1))
    val incProts:Seq[(String, BSONInteger)] = for (e <- record.entry.toSeq; v = e.settings.protect) yield (s"hourly.${hour}.settings.protect.${v.toString}" -> BSONInteger(1))
    val incInNews:Seq[(String, BSONInteger)] = for (e <- record.entry.toSeq; v = e.settings.inNews) yield (s"hourly.${hour}.settings.inNews.${v.toString}" -> BSONInteger(1))
    val incInIndex:Seq[(String, BSONInteger)] = for (e <- record.entry.toSeq; v = e.settings.inIndex) yield (s"hourly.${hour}.settings.inIndex.${v.toString}" -> BSONInteger(1))
